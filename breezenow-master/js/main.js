// Global variables
const API_KEY = '070ab13926144186af154642253008'; // Consider moving this to a secure environment variable
...
async function getWeatherReport(searchCity) {
  try {
    ...
    localStorage.setItem('fc-city',searchCity);
  } catch (error) {
...
function updateCurrentWeather(data) {
  elements.city.innerHTML = `<i class="fa-sharp fa-solid fa-location-dot"></i>${data.location.name},`;
  elements.country.innerText = `${data.location.country}`;
...
elements.darkModeToggle.addEventListener('change', function () {
  ...
  localStorage.setItem('fc-theme', isDarkMode ? 'enabled' : null);
});

// Initialize dark mode from local storage
if (localStorage.getItem('fc-theme') === 'enabled') {
  toggleDarkMode();
...
getWeatherReport(localStorage.getItem("fc-city") ?? 'New Delhi');